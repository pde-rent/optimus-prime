...
| **[oh-my-pi](https://github.com/can1357/oh-my-pi)** (omp) | Three ideas, each taken small. Hash-anchored editing: a read prints `[path#TAG]` with numbered lines and an edit carries that tag, so a file that moved underneath the model is rejected rather than silently corrupted. Verification through the project's own checker rather than a resident language server — which is what their own tooling falls back to the moment you ask a whole-project question. And suppressing repeated agent messages, from the emission guard their advisor grew after logging 309 sends of 92 distinct messages. | `edit.src` / `edit.patch`, the `check` skill, `agent-messages.ts` |
