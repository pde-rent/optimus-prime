...
| **[oh-my-pi](https://github.com/can1357/oh-my-pi)** (omp) | Hash-anchored editing. A read prints `[path#TAG]` with numbered lines, and an edit carries that tag so a file that moved underneath the model is rejected rather than silently corrupted. Their hashline is a text patch DSL with a tokenizer, parser and grammar; in a REPL the arguments are already the parse tree, so only the idea was taken. | `edit.src` / `edit.patch` |
