/**
 * East Asian Width lookup — replaces the `get-east-asian-width` package so this
 * package ships zero runtime dependencies. Terminal column widths are layout-
 * and accessibility-critical, so the table below is generated verbatim from
 * that package's Unicode data (Fullwidth + Wide ranges, merged and sorted) and
 * was diffed against it over every code point U+0000..U+10FFFF (0 mismatches)
 * before the dependency was dropped. test/east-asian-width.test.ts pins the
 * category and range boundaries.
 *
 * Source data: get-east-asian-width (MIT, Sindre Sorhus), Unicode EastAsianWidth.txt.
 */

// Flattened [start, end] pairs, sorted ascending and non-overlapping.
// biome-ignore format: compact generated table
...
// Most CJK text falls in the ideograph block; check it before binary search.
const FAST_PATH_START = 0x4e00;
const FAST_PATH_END = 0x9fff;

/**
 * Returns 2 for East Asian Fullwidth/Wide code points, 1 otherwise.
 * Ambiguous-width code points are treated as narrow, matching the default
 * behaviour of `get-east-asian-width`'s `eastAsianWidth(codePoint)`.
 */
export function eastAsianWidth(codePoint: number): number {
	if (codePoint < 0x1100) {
		return 1;
	}
	if (codePoint >= FAST_PATH_START && codePoint <= FAST_PATH_END) {
		return 2;
	}
	let low = 0;
