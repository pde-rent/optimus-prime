...
```
~/.optimus/agent/sessions/<session-id>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.optimus/agent/sessions/`.

Optimus Prime also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, Optimus Prime uses the `trash` CLI to avoid permanent deletion.
...
### Base Message Types (from Optimus Prime AI)
...
### Extended Message Types (from Optimus Prime Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by Optimus Prime (legacy field name)
