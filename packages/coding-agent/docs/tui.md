> Optimus Prime can create TUI components. Ask it to build one for your use case.
...
Optimus Prime shows compact image metadata and does not emit terminal graphics for images.
...
Customize the inline working indicator shown while Optimus Prime is streaming a response.
...
// Restore Optimus Prime's default spinner
ctx.ui.setWorkingIndicator();
```
