...
Optimus Prime separates terminal presentation, process coordination, agent execution, model-facing code execution, and persisted state. Normal interactive sessions use the daemon-backed path below; explicit SDK and fallback integrations can run the same `AgentSessionRuntime` in process.
...
flowchart LR
    ...
    subgraph worker["Session worker · one root session tree"]
        ...
        scheduler["Scheduler"]
        repl["Root Bun REPL<br/>child process"]
        children["RLM child runtimes<br/>session + optional REPL"]
...
sequenceDiagram
    ...
    participant K as Bun REPL child
...
- [RLM Runtime Architecture](rlm-runtime.md) follows Bun REPL host requests and recursive child execution.
