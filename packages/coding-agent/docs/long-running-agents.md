...
Optimus Prime combines daemon-backed session workers with persistent state, scheduled prompts, direct agent messaging, goals, and bounded autonomous continuations. These features serve different purposes but share the same session and worker runtime.
...
flowchart TD
    ...
    subgraph worker["Resident session worker"]
        ...
        repl["Persistent Bun REPL<br/>child process"]
...
The client can detach at any point. The resident worker continues to own the queue, schedules, session, REPL child, descendants, and persisted state.

## Daemon-Backed Sessions

Normal interactive sessions run in resident worker processes managed by a local supervisor. The worker owns the root session, its Bun REPL child process, scheduled jobs, and RLM descendants.
...
```bash
optimus agents                  # Open the agents view
optimus rename <agent> <name>   # Give an agent a stable readable name
optimus stop <agent>            # Stop one agent
optimus status                  # Inspect background services
optimus doctor [--fix]          # Diagnose or repair service state
optimus shutdown [--force]      # Stop all agents and services
```
...
```bash
optimus send <agent> "Please verify the latest migration"
```

From inside a REPL cell, use the preloaded `agent_message` skill:

```js
const roster = await agent_message.list_agents();
const receipt = await agent_message.send("Recheck the endpoint after the latest edit", {
...
const child = children.find((item) => item.session_name === "api-reviewer");
await agent_message.send("Continue with the updated diff", {
  receiver_role: "child",
  receiver_name: child.session_name,
});
```

Delivery is always steering: a busy target sees the message during its active run, and an idle target receives it into its context immediately.

A receipt is `delivered` when it reached an idle target's context or `queued` when a steering message was accepted and will be delivered once the target's current work allows. `agent_message.send("all", { broadcast_message: message })` broadcasts only within the family roster and returns `{ receipts: [...] }` in roster order. The daemon derives sender identity and enforces message-size, rate, and pending-queue limits.

## Heartbeats and Scheduled Prompts

Optimus Prime has three related scheduling surfaces:
...
| `optimus schedule` | User or automation | General one-time or cron prompts targeted at an agent. |
...
```js
const first = await rlm_heartbeat.create("check whether the test run finished", {
  interval: "5m",
  label: "tests",
});
const second = await rlm_heartbeat.create("inspect the deployment status", {
  interval: "10m",
  label: "deploy",
...
RLM heartbeats are distinct from the user's `/heartbeat`; the skill cannot replace or clear the user-owned heartbeat.
...
```bash
optimus schedule add worker "in 30m" -- "Check the benchmark result"
optimus schedule add worker "0 9 * * 1-5" -- "Review open work"
optimus schedule list --all
...
The model uses the REPL-side `goal` skill to inspect or finish the objective:
...
Autonomous mode is a bounded host policy for runs where no human input is expected. Optimus Prime adds follow-up continuations until configured quality gates pass or a continuation, turn, token, or wall-clock limit is reached.
...
Autonomous mode supports limits for continuations, assistant turns, tokens, and wall-clock duration. Gate commands run before the session may finish; a failed gate returns its bounded output to the agent for another attempt. Optimus Prime avoids rerunning the same failed gate when the workspace has not changed.
...
Automatic compaction handles context growth during long tasks. On overflow or near the configured threshold, Optimus Prime summarizes older messages, retains recent context, and continues. The Bun REPL child persists through compaction, so variables, imports, helper functions, and task state remain available.
