> Optimus Prime can create themes. Ask it to build one for your setup.
...
Optimus Prime loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.optimus/agent/themes/*.json`
- Project: `.optimus/agent/themes/*.json`
...
On first run, Optimus Prime detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.optimus/agent/themes
vim ~/.optimus/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/PrimeIntellect-ai/optimus/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, Optimus Prime reloads it automatically for immediate visual feedback.
...
Optimus Prime uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Optimus Prime falls back to the nearest approximation.
