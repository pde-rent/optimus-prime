...
- `web3` - JS-backed, three subsystems under one nested binding. `web3.rpc` is JSON-RPC 2.0 over HTTP for any node or chain (`await web3.rpc.call(chain|url, method, params)`, `await web3.rpc.batch(...)` for N calls in one round trip) with live endpoint discovery, transparent failover and exact BigInt unit conversion; `web3.portfolio.balances(address)` reads one wallet across EVM, Solana and Tron; `web3.defi` carries DefiLlama TVL and GeckoTerminal DEX volume. No ABI encoding, no signing.
