...
command); this skill is the REPL-side interface to it. Call it directly:

```js
await refine.status();
await refine.run();
await refine.run({ instructions: "create a memory about always checking git status before committing" });
await refine.run({ instructions: "promote the error-handling pattern to a global skill", global: true });
```

## API

- `await refine.status()` — current refine state: `pending` (whether a
