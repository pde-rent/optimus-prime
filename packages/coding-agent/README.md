...
Optimus Prime began as a hard fork of [pi-mono](https://github.com/badlogic/pi-mono), but it is now developed and distributed independently. This workspace retains inherited `@earendil-works/pi-*` source package identifiers, the `pi` package manifest key, and a source-package `pi` bin entry for internal compatibility. Public releases are currently versioned tarball artifacts installed by the scripts below; release packaging rewrites the application package and command to `optimus`. Do not use the inherited npm package as the Optimus Prime install path.
...
- [Customization](#customization)
  ...
  - [Themes](#themes)
  - [Optimus Prime Packages](#optimus-packages)
...
Then just talk to Optimus Prime. By default, Optimus Prime gives the model one tool, named `repl`: a persistent Bun JavaScript/TypeScript REPL. The model uses it to read files, run commands, edit code, and inspect data. Add capabilities via [skills](#skills), [prompt templates](#prompt-templates), [extensions](#extensions), or [Optimus Prime packages](#optimus-packages).
...
For each built-in provider, Optimus Prime maintains a list of tool-capable models, updated with every release. Authenticate via subscription (`/login`) or API key, then select any model from that provider via `/model` (or Ctrl+L).
...
**Custom providers & models:** Add providers via `~/.optimus/agent/models.json` if they speak a supported API (OpenAI, Anthropic, Google). For custom APIs or OAuth, use extensions. See [docs/models.md](docs/models.md) and [docs/custom-provider.md](docs/custom-provider.md).
...
See `/hotkeys` for the full list. Customize via `~/.optimus/agent/keybindings.json`. See [docs/keybindings.md](docs/keybindings.md).
...
On Windows Terminal, `Alt+Enter` is fullscreen by default. Remap it in [docs/terminal-setup.md](docs/terminal-setup.md) so Optimus Prime can receive the follow-up shortcut.
...
Sessions auto-save as flat JSONL files under `~/.optimus/agent/sessions/`. Each session header records its working directory, which the searchable session view uses to identify and open saved sessions.

```bash
optimus -c                  # Continue most recent session
optimus -r [path|id]        # Browse past sessions or resume one directly
optimus --no-session        # Ephemeral mode (don't save)
optimus --fork <path|id>    # Fork specific session file or ID into a new session
```
...
| `.optimus/agent/settings.json` | Project (overrides global) |

See [docs/settings.md](docs/settings.md) for all options.

### Update checks

Optimus Prime stable builds fetch `https://pub-728493de92a943e2a9b2d17b4719f318.r2.dev/latest.json` to check whether a newer version exists. Beta builds fetch `beta.json` and remain on the beta channel. Override the base URL with `OPTIMUS_DOWNLOAD_BASE_URL`. Disable version checks with `PI_SKIP_VERSION_CHECK=1`.
...
- `~/.optimus/agent/AGENTS.md` (global)
...
Replace the default system prompt with `.optimus/agent/SYSTEM.md` (project) or `~/.optimus/agent/SYSTEM.md` (global). Append without replacing via `APPEND_SYSTEM.md`.
...
```markdown
<!-- ~/.optimus/agent/prompts/review.md -->
Review this code for bugs, security issues, and performance problems.
Focus on: {{focus}}
```

Place in `~/.optimus/agent/prompts/`, `.optimus/agent/prompts/`, or a [Optimus Prime package](#optimus-packages) to share with others. See [docs/prompt-templates.md](docs/prompt-templates.md).

### Skills

On-demand capability packages following the [Agent Skills standard](https://agentskills.io). At startup, Optimus Prime gives the model each visible skill's name, type, description, and location. The full `SKILL.md` stays out of context until the model inspects it with `repl` or you explicitly invoke `/skill:name`.

```markdown
<!-- ~/.optimus/agent/skills/my-skill/SKILL.md -->
---
name: my-skill
...
Skills can also be JS-backed. A JS skill is a normal skill directory with `SKILL.md` plus a `skill.js` ESM module at its root that exports a `createSkill(ctx)` factory. Optimus Prime preloads it into the persistent REPL and binds whatever the factory returns under the skill's name with hyphens replaced by underscores, so the model can call it directly:

```js
// ~/.optimus/agent/skills/word-count/skill.js
...
Credentials are stored once in `~/.optimus/agent/auth.json` (under `mcp:<name>`); the skill reads them directly from the REPL and asks the host to refresh expired tokens. Enablement is derived from whether valid credentials exist, so there is no separate on/off switch.
...
```js
// ~/.optimus/agent/skills/acme/skill.js
...
TypeScript modules that extend Optimus Prime with custom tools, commands, keyboard shortcuts, event handlers, and UI components.
...
The default export can also be `async`. Optimus Prime waits for async extension factories before startup continues, which is useful for one-time initialization such as fetching remote model lists before calling `pi.registerProvider()`.
...
- Make Optimus Prime look like Claude Code
- Games while waiting (yes, Doom runs)
- ...anything you can dream up

Place in `~/.optimus/agent/extensions/`, `.optimus/agent/extensions/`, or a [Optimus Prime package](#optimus-packages) to share with others. See [docs/extensions.md](docs/extensions.md) and [examples/extensions/](examples/extensions/).

### Themes

Built-in: `dark`, `light`. Themes hot-reload: modify the active theme file and Optimus Prime immediately applies changes.

Place in `~/.optimus/agent/themes/`, `.optimus/agent/themes/`, or a [Optimus Prime package](#optimus-packages) to share with others. See [docs/themes.md](docs/themes.md).

### Optimus Prime Packages

Bundle and share extensions, skills, prompts, and themes via npm or git.

> **Security:** Optimus Prime packages run with full system access. Extensions execute arbitrary code, and skills can instruct the model to perform any action including running executables. Review source code before installing third-party packages.

```bash
optimus package install npm:@foo/optimus-tools
optimus package install npm:@foo/optimus-tools@1.2.3  # pinned version
optimus package install git:github.com/user/repo
optimus package install git:github.com/user/repo@v1       # tag or commit
optimus package install git:git@github.com:user/repo
optimus package install https://github.com/user/repo
optimus package install ssh://git@github.com/user/repo
optimus package remove npm:@foo/optimus-tools
optimus package list
optimus package update                                  # update packages, except pinned versions
optimus package update npm:@foo/optimus-tools       # update one package
optimus update                                          # update Optimus Prime
optimus update --force                                  # reinstall Optimus Prime even if current
optimus config                                          # enable/disable package resources
```

Packages install to `~/.optimus/agent/git/` (git) or the global package root. Use `--local` for project-local installs (`.optimus/agent/git/`, `.optimus/agent/npm/`). Git packages install dependencies with `bun install --production` by default, so runtime deps must be listed under `dependencies`. To install through a different package manager or a wrapper, set `npmCommand` in `settings.json`, for example `["mise", "exec", "node@20", "--", "npm"]`.

Create a package by adding the inherited `pi` manifest key to `package.json`:

```json
{
  "name": "my-optimus-package",
  "keywords": ["optimus-package"],
  "pi": {
    "extensions": ["./extensions"],
    "skills": ["./skills"],
    "prompts": ["./prompts"],
...
Without a `pi` manifest, Optimus Prime auto-discovers from conventional directories (`extensions/`, `skills/`, `prompts/`, `themes/`).
...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "optimus";
...
Optimus Prime is forked from [pi-mono](https://github.com/badlogic/pi-mono) by Mario Zechner and keeps MIT attribution in the root license.

The package architecture, extension model, and source package names still reflect that upstream lineage while the distributed command and release artifacts are branded for Optimus Prime.
...
Run `optimus help` for the command list and `optimus help <command>` for details.

### Agent Commands

```bash
optimus agents                         # Search running, idle, and inactive sessions
optimus list [--all]                   # List active or saved agents
optimus attach <agent>                 # Attach the interactive UI
optimus stop <agent>                   # Stop one agent
optimus rename <agent> <name>          # Rename an agent
optimus send <agent> <message>         # Send an agent-to-agent message
optimus status                         # Show background service status
optimus doctor [--fix]                 # Inspect or safely clean up background services
optimus shutdown [--force]             # Stop every agent, worker, and background service
```
...
```bash
optimus schedule list [--all] [agent]
optimus schedule add <agent> <schedule> -- <message>
optimus schedule cancel <job-id>
```
...
optimus update [--force]                   # Update Optimus Prime itself
optimus config                             # Enable/disable package resources
```
...
Use `optimus session export <file> [output]` to export a saved session to HTML.
...
# High thinking level
optimus --thinking high "Solve this complex problem"
```
...
|----------|-------------|
| `OPTIMUS_CODING_AGENT_DIR` | Override config directory (default: `~/.optimus/agent`) |
...
| `OPTIMUS_TELEMETRY` | Override pseudonymous aggregate usage analytics with `1`/`true`/`yes` or `0`/`false`/`no` |
| `OPTIMUS_TELEMETRY_ENDPOINT` | Override the aggregate analytics ingestion endpoint |
...
| `OPTIMUS_REPL_SKILLS` | **Host-set, not user-set.** JSON list of the JS skills the REPL child preloads (`{name, global, entry}`). Optimus Prime writes it when spawning the REPL; setting it yourself is unsupported |
| `VISUAL`, `EDITOR` | External editor for Ctrl+G |

The remaining `PI_*` variables in this table are compatibility names still read by the current runtime. They do not change the application name, command, or default `~/.optimus/agent` configuration path.
...
- [Optimus Prime AI](../ai): Core LLM toolkit
- [Optimus Prime Core](../agent): Agent framework
- [Optimus Prime TUI](../tui): Terminal UI components
