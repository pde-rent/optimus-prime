...
describe("parseArgs", () => {
	...
	describe("flags with values", () => {
		...
		test("rejects removed --export syntax", () => {
			...
			expect(result.diagnostics).toContainEqual({
				type: "error",
				message: '--export was removed. Use "optimus session export <file> [output]".',
			});
		});
		...
		test("rejects removed --list-models syntax", () => {
			...
			expect(result.diagnostics).toContainEqual({
				type: "error",
				message: '--list-models was removed. Use "optimus model list [search]".',
			});
		});
	...
	describe("tool flags", () => {
		...
		test("parses --tools flag", () => {
			const result = parseArgs(["--tools", "repl,dynamic_tool"]);
			expect(result.tools).toEqual(["repl", "dynamic_tool"]);
		});

		test("parses -t shorthand", () => {
			const result = parseArgs(["-t", "repl,dynamic_tool"]);
			expect(result.tools).toEqual(["repl", "dynamic_tool"]);
		});

		test("parses --no-tools with explicit --tools flags", () => {
			const result = parseArgs(["--no-tools", "--tools", "repl,dynamic_tool"]);
			expect(result.noTools).toBe(true);
			expect(result.tools).toEqual(["repl", "dynamic_tool"]);
		});

		test("parses --no-builtin-tools with explicit --tools flags", () => {
			const result = parseArgs(["--no-builtin-tools", "--tools", "repl,dynamic_tool"]);
			expect(result.noBuiltinTools).toBe(true);
			expect(result.tools).toEqual(["repl", "dynamic_tool"]);
		});
