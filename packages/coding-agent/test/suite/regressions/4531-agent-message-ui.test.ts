...
import type { KernelSentAgentMessage } from "../../../src/core/tools/repl-types.js";
...
type LateSentAgentMessageHost = {
	_recordLateReplSentAgentMessage: (toolCallId: string, message: KernelSentAgentMessage) => void;
	_agentEventQueue: Promise<void>;
	_lateReplSentAgentMessages: Map<string, KernelSentAgentMessage[]>;
	_restoreLateReplSentAgentMessages: () => void;
};
...
describe("ENG-4531 agent message UI", () => {
	...
	it("persists sent messages that arrive after their REPL cell completes", async () => {
		const harness = await createHarness();
		harnesses.push(harness);
		const toolResult: ToolResultMessage = {
			role: "toolResult",
			toolCallId: "repl_4531",
			toolName: "repl",
		...
		harness.session.sessionManager.appendMessage(
			fauxAssistantMessage(fauxToolCall("repl", { code: "background_send" }), { stopReason: "toolUse" }),
		);
		...
		host._recordLateReplSentAgentMessage(toolResult.toolCallId, lateMessage);
		...
		expect(
			harness.session.sessionManager
				.getEntries()
				.some((entry) => entry.type === "custom" && entry.customType === "repl_sent_agent_message"),
		).toBe(true);
		expect(events).toContain("repl_sent_agent_message");
		expect(
			harness.session
				.buildSessionContext()
				.messages.find(
		...
		host._restoreLateReplSentAgentMessages();
		expect(toolResult.details).toMatchObject({ sentAgentMessages: [lateMessage] });

		toolResult.details = { status: "ok" };
		host._lateReplSentAgentMessages = new Map();
		host._restoreLateReplSentAgentMessages();
		expect(toolResult.details).toMatchObject({ sentAgentMessages: [lateMessage] });

		host._lateReplSentAgentMessages.set("repl_other_branch", [
			{
		...
		host._restoreLateReplSentAgentMessages();
		expect(host._lateReplSentAgentMessages.has("repl_other_branch")).toBe(false);
	});
	...
	it("uses compact rebuilt spacing for agent messages next to messages and tool cells", () => {
		...
		const toolCall = fauxAssistantMessage(fauxToolCall("repl", { code: "print('ready')" }), {
			stopReason: "toolUse",
		});
		const options = {
			ui: { requestRender: () => {} } as unknown as TUI,
		...
		expect(isCompactAgentMessageNeighbor(new ReplCellComponent({ code: "print('direct')" }))).toBe(true);
	...
	it("suppresses live spacing between agent messages and following tool activity", () => {
		...
		addMessage(fauxAssistantMessage(fauxToolCall("repl", { code: "print('live')" }), { stopReason: "toolUse" }));
		expect(chatContainer.children[2]?.render(120)[0]).not.toBe("");

		const toolComponents = buildConversationComponents(
			[fauxAssistantMessage(fauxToolCall("repl", { code: "print('ready')" }), { stopReason: "toolUse" })],
			{
				ui: { requestRender: () => {} } as unknown as TUI,
				cwd: "/tmp",
				toolOptions: {},
	...
	it("renders sent messages beneath collapsed REPL cells", () => {
	...
	it("decouples sent-message expansion from tool-output expansion", () => {
		...
		const agentExpanded = stripAnsi(
			new ReplCellComponent({ ...baseState, expanded: false, agentMessagesExpanded: true }).render(120).join("\n"),
		);
		...
		const toolExpanded = stripAnsi(
			new ReplCellComponent({ ...baseState, expanded: true, agentMessagesExpanded: false }).render(120).join("\n"),
		);
