...
import { type AssistantMessage, fauxAssistantMessage, fauxToolCall, Type, type Usage } from "@earendil-works/pi-ai";
...
/**
 * Stand-in for the real repl tool. Goal calls reach the host over the
 * kernel comm bridge while an repl cell executes; this stub mirrors that
 ...
 */
function createFauxReplTool(sessionRef: { current?: AgentSession }): AgentTool {
	return {
		name: "repl",
		label: "repl",
...
describe("AgentSession goals", () => {
	...
	it("keeps continuing until the model completes the goal through repl", async () => {
		...
		harness.setResponses([
			...
			fauxAssistantMessage(fauxToolCall("repl", COMPLETE_GOAL_CELL), { stopReason: "toolUse" }),
	...
	it("counts tokens from the goal completion turn", async () => {
		const harness = await createGoalHarness();
		harness.setResponses([
			assistantWithUsage(fauxAssistantMessage(fauxToolCall("repl", COMPLETE_GOAL_CELL), { stopReason: "toolUse" }), {
				input: 4,
				output: 2,
				totalTokens: 6,
			}),
	...
	it("activates repl when a slash goal starts from an inactive tool set", async () => {
		const harness = await createGoalHarness();
		...
		expect(harness.session.getActiveToolNames()).toEqual(["repl"]);
		expect(harness.session.goalState).toMatchObject({
			active: false,
			status: "complete",
		});
	...
	it("adds repl to the live continuation context when inactive at run start", async () => {
		const harness = await createGoalHarness();
	...
	it("keeps repl active on active-goal runtime rebuild", async () => {
		const harness = await createGoalHarness();
		harness.session.handleGoalHostRequest("goal.create", { objective: "finish the active goal" });
	...
	it("lets the model create a persistent goal through repl", async () => {
		const harness = await createGoalHarness();
		harness.setResponses([
			fauxAssistantMessage(fauxToolCall("repl", { code: 'goal.create {"objective": "write a benchmark note"}' }), {
				stopReason: "toolUse",
			}),
