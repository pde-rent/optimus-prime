	...
	it("renders a multiline prompt with compact option rows", () => {
		const selector = new ExtensionSelectorComponent(
			[
				...
				"Waiting preserves the current kernel state. Killing restarts the REPL and loses in-memory variables.",
			].join("\n"),
		...
		expect(output).toContain("Interrupted REPL cell is still running");
