...
const PROCESS_STRESS_WORKERS = Number.parseInt(process.env.OPTIMUS_STRESS_WORKERS ?? "10", 10);
...
`,
	...
	it("keeps client-owned workers hidden and removes them without archiving", async () => {
		...
		const created = await client.request({
			...
			launchEnv: { OPTIMUS_OWNED_TEST: launchEnvSentinel },
			config: { cwd: projectDir, agentDir, sessionDir, noTools: true, noExtensions: true },
		});
		expect(created.success).toBe(true);
	...
	it.skipIf(!hasTag("process-stress"))("cancels an archived session heartbeat without spawning a worker", async () => {
		const root = tempDir();
		const agentDir = join(root, "agent");
		const projectDir = join(root, "project");
		const sessionDir = join(agentDir, "sessions");
	...
	it.skipIf(!hasTag("process-stress"))(
	...
	it.skipIf(!hasTag("process-stress"))("survives a worker process spawn error", async () => {
		const root = tempDir();
		const agentDir = join(root, "agent");
		const projectDir = join(root, "project");
	...
	it("isolates a root, streams a chunked snapshot, and adopts the same worker after restart", async () => {
		...
		expect(readFileSync(sessionFile, "utf8")).toContain("optimus.worker_recovery");
