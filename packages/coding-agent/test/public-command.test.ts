...
describe("public command routing", () => {
	...
	it("rejects extra attach operands", async () => {
		...
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining(`${APP_NAME} attach <agent>`));
	});
	...
	it("separates Optimus Prime updates from package updates", async () => {
	...
	it("gives legacy update targets explicit migration guidance", async () => {
		for (const target of ["self", "--self", "optimus"]) {
			await handlePublicCommand(["update", target]);
		}

		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining(`Use "${APP_NAME} update [--force]"`));
	});
	...
	it("directs legacy package-update forms to the package command", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining(`Use "${APP_NAME} package update [source]"`));
	});
	...
	it("rejects self-update aliases on the package update path", async () => {
		for (const source of ["self", "pi", "optimus"]) {
			await handlePublicCommand(["package", "update", source]);
		}

		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining(`Use "${APP_NAME} update"`));
	});
	...
	it("directs package uninstall to package remove", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining(`Use "${APP_NAME} package remove"`));
	...
	it("rejects operands for package list", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining(`${APP_NAME} package list`));
	});
	...
	it("rejects the old daemon hierarchy with migration guidance", async () => {
		...
		expect(process.exitCode).toBe(1);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining(`Run "${APP_NAME} help"`));
	});

	it("shows migration guidance when help targets removed commands", async () => {
		const cases: Array<[path: string[], hint: string]> = [
			[["daemon"], `Run "${APP_NAME} help"`],
			[["install"], `Use "${APP_NAME} package install"`],
			[["remove"], `Use "${APP_NAME} package remove"`],
			[["uninstall"], `Use "${APP_NAME} package remove"`],
			[["manage"], `Use "${APP_NAME} agents"`],
			[["app", "update"], `Use "${APP_NAME} update"`],
		];
	...
	it("shows command help when options precede the help flag", async () => {
		...
		expect(console.log).toHaveBeenNthCalledWith(1, expect.stringContaining(`${APP_NAME} list [--all] [--json]`));
		expect(console.log).toHaveBeenNthCalledWith(2, expect.stringContaining(`${APP_NAME} doctor [--fix] [--json]`));
		expect(console.log).toHaveBeenNthCalledWith(3, expect.stringContaining(`${APP_NAME} package install <source>`));
	...
	it("formats complete top-level help, including autonomous options", () => {
		...
		expect(help).toContain("--mode <text|json|rpc|daemon>");
