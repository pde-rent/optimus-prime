...
const skillReference = {
	...
	binding: "example",
	callable: "run",
	call_pattern: "await example.run(...)",
};
...
describe("harness refinement", () => {
				...
				edits: [
					...
					},
					{
						...
						reference: {
							...
							callable: "run",
							call_pattern: "await native_check.run({ command })",
						},
						arguments: {
	...
	it("requires JS references for harness-created skills", () => {
		...
		const nonJsReference = applyRefinementProposal(
			state,
		...
		expect(nonJsReference.appliedEdits[0]).toMatchObject({
			applied: false,
...
`,
	...
	it("plans a proposal without mutating harness state", async () => {
		...
		expect(request.systemPrompt).toContain('handle = await spawn("<task>")');
