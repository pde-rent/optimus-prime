...
const REFINEMENT_SYSTEM_PROMPT = `You are Optimus Prime's /refine continual harness subsystem.
...
skills, and subagent specs that lets Optimus Prime improve reusable behavior
outside the token history.
Use "continual harness" for that persistent artifact layer; keep "RLM" for the
runtime, JavaScript/TypeScript REPL, and native call interface that executes those artifacts.
...
- Use memory for declarative facts and preferences, skill for repeatable procedures exposed as JS calls, prompt for narrow behavioral policy addendums, and subagent for reusable delegation roles.
  ...
  "edits": [
    {
      ...
      "reference": {"type": "js", "binding": "skill_binding", "callable": "method_name", "call_pattern": "await skill_binding.method_name(...)"},
...
const AUTO_REFINE_REVIEW_SYSTEM_PROMPT = `You are Optimus Prime's automatic /refine review gate.
...
function validateEdit(edit: RefinementEdit, computedId?: string): string | undefined {
	...
	if (edit.action !== "delete" && edit.kind === "skill") {
		...
		if (!reference) {
			return `${edit.action} skill requires js reference`;
		}
		if (reference.type !== "js") {
			return `${edit.action} skill reference.type must be js`;
		}
		const hasBinding =
			(typeof reference.binding === "string" && reference.binding.length > 0) ||
			(typeof reference.skill_binding === "string" && reference.skill_binding.length > 0);
		const hasCallable =
		...
		if (!hasBinding) {
			return `${edit.action} skill requires js binding`;
		}
		if (!hasCallable) {
...
export async function planRefinement(
	...
	const scopeInstruction = options.global
		? "Requested refinement scope: global. Only propose stable cross-session continual harness edits, durable user preferences, reusable skills/subagents, or explicitly project-qualified facts that should affect future Optimus Prime sessions. Do not persist session-only progress, temporary blockers, or current-run coordination globally."
		: "Requested refinement scope: local. Prefer local continual harness edits for current task progress, temporary blockers, current-run coordination, and project facts that are not clearly reusable across Optimus Prime sessions. Global entries in the overview are read-only context: do not propose update or delete edits for them; create a local entry instead if an override is needed.";
	const userPrompt = [
