/**
 * Extension loader - loads TypeScript extension modules with Bun's native
 * importer plus virtual-module shims (see native-import.ts).
 */
...
async function loadExtensionModule(extensionPath: string) {
	// jiti is gone: native Bun import handles TS transpile and type-erasure,
	// and the virtual-module shims remap @pi/* specifiers to shared instances.
	const { VIRTUAL_MODULES } = await import("./bundled-modules.js");
	const namespace = (await importExtensionModule(extensionPath, VIRTUAL_MODULES)) as {
		default?: unknown;
	};
	const factory = namespace.default;
	return typeof factory !== "function" ? undefined : (factory as ExtensionFactory);
}
