...
export const DEFAULT_RLM_RUNTIME_LABELS = [
	...
	"`search()` and `ls()` — native repo inspection helpers callable only inside a repl cell (there is no standalone `search` or `ls` tool): `search(regexSource, { glob?, cwd?, maxResults?, maxCharsPerLine? })` returns `{ file, line, text }[]` grouped per file and capped overall; `ls(glob?, { cwd?, limit? })` lists files. Both skip node_modules/.git/dist and binary files by default. For any 'where is X defined' or 'which files mention Y' question about this repo, reach for these first — before the grep/find tools, shell grep/find/ls, or hand-rolled directory walks",
