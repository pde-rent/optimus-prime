...
export function isInjectedPromptMessage(message: AgentMessage): message is InjectedPromptMessage {
	return (
		...
		(message.customType === HEARTBEAT_PROMPT_CUSTOM_TYPE ||
			...
			message.customType === REPL_STATE_RESTORED_CUSTOM_TYPE ||
...
export class InjectedPromptMessageComponent extends Container {
	...
	private updateDisplay(): void {
		...
		if (this.expanded && this.message.customType !== REPL_STATE_RESTORED_CUSTOM_TYPE) {
	...
	private headerText(): string {
		...
		if (this.message.customType === REPL_STATE_RESTORED_CUSTOM_TYPE) {
			const details = this.message.details as ReplStateRestoredDetails | undefined;
			const label = details?.restored === false ? "Started fresh REPL kernel" : "Restored REPL kernel state";
