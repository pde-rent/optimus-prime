...
import type { PrimeAgentReplMeta, PrimeAgentSessionMeta } from "./acp-meta.js";
...
/** prime-agent's model-facing tool is the `repl` REPL; bash is the secondary escape hatch. */
...
/** Extract the REPL cell source so a client can show what is executing. */
...
/**
 * Rich REPL output that ACP has no content type for.
 *
 * The repl tool reports media and diffs under `details` (images additionally
 ...
 */
function replRichOutput(result: unknown): PrimeAgentReplMeta | undefined {
	if (!result || typeof result !== "object") return undefined;
	const details = (result as { details?: unknown }).details;
	if (!details || typeof details !== "object") return undefined;
	const { attachments, diffs } = details as { attachments?: unknown; diffs?: unknown };
	const meta: PrimeAgentReplMeta = {};
...
export function acpUpdatesForSessionEvent(
	...
	switch (event.type) {
		...
		case "tool_execution_start": {
			const cell = event.toolName === REPL_TOOL_NAME ? replCellSource(event.args) : undefined;
			return [
				{
					sessionUpdate: "tool_call",
					toolCallId: event.toolCallId,
					title: event.toolName === REPL_TOOL_NAME ? "REPL cell" : event.toolName,
		...
		case "tool_execution_end": {
			...
			const rich = event.toolName === REPL_TOOL_NAME ? replRichOutput(event.result) : undefined;
			return [
				{
					...
					...(rich ? { _meta: primeAgentMeta({ repl: rich }) } : {}),
				},
			];
		}
