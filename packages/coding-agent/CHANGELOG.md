...
- Fixed skill loading silently mangling a SKILL.md whose `description` contains an unquoted `": "`: the loader now recovers the literal text from the raw frontmatter, warns `description contains ": " - wrap the value in quotes`, and falls back to the directory name (with a warning) when `name` parses to a non-string; every skipped skill now reports why through the skill diagnostics surface.
- Added batched reads: `read_file` and `head`/`tail` accept `paths[]` (up to 16) to fetch many files in ONE call; the tool sizes them first, reads newest-mtime-first within `limitBytes` total (default 100KB), renders each file under a `=== <path> (N bytes) ===` header with the unchanged short-circuit applied per file, skips missing paths with a note, and returns only the size table when nothing fits.
- Added a native-tool nudge to the bash tool: when a single simple command (no pipes or chaining) matches a covered in-process tool (`grep`, `cat`, `echo` redirecting to a file, `sed`, `find`, `wc`, `head`, `tail`, `ln`, `ps`/`top`/`df`/`free`/`uname`, `ss`/`netstat`/`ifconfig`), the result gains a one-line tip pointing at the native tool, shown at most once per session per command.
...
- Added an unchanged short-circuit to `read_file`: re-reading a file with the same range when mtime, size and content hash are all unchanged returns a one-line `[file unchanged since your last read: lines X-Y of Z]` notice instead of re-emitting content (a full 50KB page costs ~13k estimated tokens; the notice costs ~15). Read state is per session and invalidated through the file mutation queue on every `write_file`/`edit`/`sed`/`ln` mutation.
...
- Changed the live subagent graph panel to a fixed eight-row contract under the root: up to six running agents nested at their real tree depth (branch prefix glued to the spinner glyph, lean de-slugged name capped at 24 columns, no task text), an `… N more running` overflow row summing the folded agents' tokens and runtime, and one aggregated `N agents done (S succeeded, F failed)` row with every finished child's total spend; right-aligned token/runtime cells are unchanged.
...
- Removed duplicated guidance from the system prompt: the subagent block no longer restates the recursion block's admission, reply, handle and observation mechanics, and installed skills are named once in the `<available_skills>` roster instead of also being listed by name. Both were paid on every request at every depth.
- Fixed `--prompt-template` and `--theme` losing to settings while `--skill` won, for otherwise identical resolution code.
- Added a built-in MCP client. It speaks both protocol eras -- the stateless `2026-07-28` revision and the handshake revisions `2025-11-25` and earlier -- probing an endpoint once and caching which it uses, so it works against the roughly seven in eight servers still on the handshake. Negotiates a mutually supported version on rejection, follows `tools/list` pagination, reads responses delivered as event streams, and mirrors parameters a server annotates with `x-mcp-header`, rejecting tools whose annotations are invalid. Runs host-side, so credentials never enter the REPL.
- Renamed the per-server tool filters to `includeTools` and `excludeTools`, matching how the wider convention names them, and implemented them: they previously had no readers at all. Exclusion takes precedence over inclusion.
- Fixed a configured stdio MCP server being silently dropped, which made it report as an unknown server. It is now listed with its transport, and asking it for tools says the transport is not reachable yet.
- Added the `mcp` REPL binding: `mcp.servers()`, `mcp.tools(server)`, `mcp.call(server, tool, args)`. Tools are not flattened into the model's tool surface, so a server with a large tool list costs nothing until it is asked and cannot collide with another server's names.
- Added a `check` skill: `await check()` detects the project by its root markers (searching upward, so a monorepo package inherits the repository's config), runs that toolchain's own checker, and reports what it says. Concurrent identical checks share one process. It reports every diagnostic rather than only new ones, because several agents can share a working tree and a checker that hid what it already mentioned would show a clean result on a project another agent just broke.
- Added duplicate suppression to model-originated agent messages: an exact repeat of a message already delivered to the same target is not delivered again, and the sender is given the original receipt so it sees success and has nothing to rephrase around. The first send of any text always lands, so a child whose complete answer is short is never lost, and host-generated notices and human `/message` sends never reach the filter.
- Added hash-anchored line editing: `edit.src(path)` prints `[path#TAG]` with numbered lines and `edit.patch(path, tag, hunks)` edits by line number, rejecting the call if the file changed since the tag was issued. Hunk numbers index the tagged snapshot so they never shift within a call, overlapping hunks and no-op patches are errors, and the new tag is returned so a chain of edits needs one read.
