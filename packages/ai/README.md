...
<h1 align="center">Optimus Prime AI</h1>

<p align="center">
  LLM provider toolkit.
</p>

Release docs use the Optimus Prime package name. The source workspace manifest still keeps an inherited package name until the namespace migration is complete.
...
TypeBox exports are re-exported from `optimus-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from 'optimus-ai';
...
```typescript
import { Type, Tool, StringEnum } from 'optimus-ai';
...
```typescript
import { stream, validateToolCall, Tool } from 'optimus-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from 'optimus-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from 'optimus-ai';
...
```typescript
import { getModel, stream } from 'optimus-ai';
...
```typescript
import { getProviders, getModels, getModel } from 'optimus-ai';
...
```typescript
import { Model, stream } from 'optimus-ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are Optimus Prime thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from 'optimus-ai';
...
```typescript
import { getModel, complete, Context } from 'optimus-ai';
...
```typescript
import { Context, getModel, complete } from 'optimus-ai';
...
- OAuth login flows are not supported in browser environments. Use the `optimus-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from 'optimus-ai';
...
```bash
bunx optimus-ai login              # interactive provider selection
bunx optimus-ai login anthropic    # login to specific provider
bunx optimus-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `optimus-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from 'optimus-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from 'optimus-ai/oauth';
...
```typescript
import { getModel, complete } from 'optimus-ai';
import { getOAuthApiKey } from 'optimus-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `optimus-ai`
