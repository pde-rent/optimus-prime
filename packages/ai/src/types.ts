...
export interface AssistantMessage {
	...
	upstreamProvider?: string; // Backend that actually served the request behind an aggregator (e.g. OpenRouter's top-level `provider`)
	systemFingerprint?: string; // Provider `system_fingerprint` identifying the serving backend configuration
...
export interface OpenAICompletionsCompat {
	...
	/** Whether the provider supports the `strict` field in tool definitions. Default: false. Enable explicitly only for providers verified to accept it. */
	supportsStrictMode?: boolean;
	/** Cache control convention for prompt caching. "anthropic" applies Anthropic-style `cache_control` markers to the system prompt, last tool definition, and last user/assistant/tool text content. */
