...
optimus_test_cols=80
optimus_test_rows=24

optimus_read_terminal_size() {
	optimus_screen_cols="$optimus_test_cols"
	optimus_screen_rows="$optimus_test_rows"
}
...
print_render_meta() {
	...
	printf '__META__ %s cols=%s rows=%s layout_show_logo=%s lab_width=%s render_lab_width=%s compact=%s visible=%s content_height=%s\\n' \\
		"$label" "$optimus_screen_cols" "$optimus_screen_rows" "$optimus_screen_layout_show_logo" \\
		"$optimus_screen_layout_lab_width" "$optimus_screen_render_lab_width" "$optimus_screen_compact" "$visible" "$content_height"
}

render_case() {
	optimus_screen_title="Installing Optimus Prime"
	optimus_screen_detail="Fetching the verified package."
	optimus_screen_question=
	optimus_screen_frame=1
	optimus_screen_cols="$1"
	optimus_screen_rows="$2"
	...
	optimus_screen_frame=2
	optimus_screen_cols="$3"
	optimus_screen_rows="$4"
	optimus_refresh_screen_layout_mode
...
screen_case() {
	...
	optimus_screen "Installing Optimus Prime" "Installing Optimus Prime" "Fetching the verified package." ""
...
Finalizing bun install."
	for progress_frame in 1 24 25 48 49 200; do
		optimus_animation_frame="$progress_frame"
		printf '__PROGRESS__ %s\t%s\t%s\\n' "$progress_frame" "$(optimus_animation_status "Installing Optimus Prime" "$progress_details" static)" "$(optimus_animation_detail "$progress_details")"
	done
}
...
const tempDir = mkdtempSync(join(tmpdir(), "optimus-installer-render-"));
...
function assertInstallerProgress(progress) {
	...
	for (const [index, expectedDetail] of expectedDetails.entries()) {
		...
		check(
			progress[index].status === "Installing Optimus Prime...",
