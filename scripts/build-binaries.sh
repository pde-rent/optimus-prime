...
if [[ "$SKIP_DEPS" == "false" ]]; then
    ...
    # A normal install only pulls optional deps for the current platform
    # We need all platform bindings for bun cross-compilation
    # --cpu/--os '*' bypass the platform checks (os/cpu restrictions in package.json)
    # Install all in one command to avoid removing packages from previous installs
    bun add --no-save --cpu='*' --os='*' \
