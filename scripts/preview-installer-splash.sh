...
frames="${1:-${OPTIMUS_SPLASH_PREVIEW_FRAMES:-300}}"
case "$frames" in
	""|*[!0-9]*)
...
preview_dir=$(mktemp -d "${TMPDIR:-/tmp}/optimus-splash-preview.XXXXXX")
...
i=0
while [ "$i" -lt "$1" ]; do
	optimus_screen "Installing Optimus Prime" "Downloading Optimus Prime$(optimus_pulse)" "Fetching the verified package." ""
	sleep 0.18
	i=$((i + 1))
done
EOF
